...
def compute_dg(pdb_path):
    try:
        out = subprocess.check_output(
            ['prodigy', '-q', pdb_path, '--selection', 'H', 'T'],
...
def match_backbone(candidate_stem, backbone_files):
    ...
    if matches:
        ...
        return stems[best]
    if backbone_files:
        print(f'WARNING: tidak ada backbone yang cocok untuk {candidate_stem}, menggunakan {os.path.basename(backbone_files[0])} sebagai fallback.')
    ...
    for pdb_path in candidate_files:
        ...
        if backbone_path and pred_atoms:
            ...
            if len(framework_res) >= 3:
                ...
                print(f'WARNING: residu framework tidak cukup untuk Kabsch alignment pada {stem}.')
        else:
            print(f'WARNING: tidak dapat menghitung CDR/H3 RMSD untuk {stem} (backbone/chain H tidak ditemukan).')
