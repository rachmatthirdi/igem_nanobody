(function () {
  ...
  function renderLine(entry) {
    ...
    const time = new Date(entry.ts).toLocaleTimeString('id-ID', { hour12: false });
  ...
  window.ConsolePanel = {
    init() {
      ...
      document.getElementById('console-header').addEventListener('click', (e) => {
        ...
        document.getElementById('console-footer').classList.toggle('collapsed');
      });
      ...
      document.getElementById('btn-console-export').addEventListener('click', () => {
        const text = lines
          .map((l) => `[${new Date(l.ts).toISOString()}] [${l.level}] [${l.source || '-'}] ${l.text}`)
          .join('\n');
